...
### BROADLY CONCEPTUALIZED SOCIAL DISCONNECTEDNESS ###
# excluding the number of friendships, relationship status, living situation and social support from the adjustment set
...
time_varying_covariates <- c("employment_", "smoking_", "alcohol_use_", "phys_exer_", "bmi_", "loneliness_", "depression_")
...
exposure_models_imp_soc_discon <- imp_UiN_data %>% 
  group_by(.imp) %>% 
  nest() %>% 
...
BMI_outcome_models_soc_discon <- exposure_models_imp_soc_discon %>% 
...
BMI_outcome_effects_soc_discon <- select(BMI_outcome_models_soc_discon, starts_with("effect")) %>% 
...
alcohol_outcome_models_soc_discon <- exposure_models_imp_soc_discon %>% 
  mutate(outcome_model_l1_b2 = map(.x = data_ext,
...
alcohol_outcome_effects_soc_discon <- select(alcohol_outcome_models_soc_discon, starts_with("effect")) %>% 
  pivot_longer(cols = effect_l1_b2_1sd:effect_l3_b4_2sd, names_to = "effect", values_to = "contrasts") %>% 
  group_by(effect) %>%  nest() %>% 
...
smoking_outcome_models_soc_discon <- exposure_models_imp_soc_discon %>% 
  mutate(outcome_model_l1_b2 = map(.x = data_ext,
...
smoking_outcome_effects_soc_discon <- select(smoking_outcome_models_soc_discon, starts_with("effect")) %>% 
  pivot_longer(cols = effect_l1_b2_1sd:effect_l3_b4_2sd, names_to = "effect", values_to = "contrasts") %>% 
  group_by(effect) %>%  nest() %>% 
  mutate(contrasts = map(.x = data, ~ summary(pool(.x$contrasts), conf.int = TRUE))) %>% 
  select(contrasts) %>% unnest() %>% 
...
phys_act_outcome_models_soc_discon <- exposure_models_imp_soc_discon %>% 
  mutate(outcome_model_l1_b2 = map(.x = data_ext,
...
# plot: causal effect of loneliness (broadly conceptualized, not adjusted for number of friendships, relationship status, living situation and social support)
phys_act_outcome_effects_soc_discon <- select(phys_act_outcome_models_soc_discon, starts_with("effect")) %>% 
  pivot_longer(cols = effect_l1_b2_1sd:effect_l3_b4_2sd, names_to = "effect", values_to = "contrasts") %>% 
  group_by(effect) %>%  nest() %>% 
  mutate(contrasts = map(.x = data, ~ summary(pool(.x$contrasts), conf.int = TRUE))) %>% 
  select(contrasts) %>% unnest() %>% 
...
### RESULTS ###
ATE_loneli_health_soc_discon <- bind_rows(BMI_outcome_effects_soc_discon, alcohol_outcome_effects_soc_discon, 
                                          smoking_outcome_effects_soc_discon, phys_act_outcome_effects_soc_discon) %>% 
  ungroup() %>% 
...
results_ATE_loneli_health_soc_discon <- ATE_loneli_health_soc_discon %>% 
...
write.csv(results_ATE_loneli_health_soc_discon, "N:/durable/Data_analyses/Libor/Loneliness_Health_Risk_Factors/Tables/results_ATE_loneli_health_soc_discon")

plot_ATE_loneli_health_soc_discon <- ATE_loneli_health_soc_discon %>%
  ...
  labs(title = "Causal Effect of Loneliness on Health Outcomes",
       subtitle = "unadjusted for social factors",
       x = "",
       y = "") +
  theme_minimal() +
...
ggsave(filename = "plot_ATE_loneli_health_soc_discon.jpeg",
